...
def test_implies():
    ...
    assert as_object(t1) == {
        'arguments': ['T1',
               {'arguments': ['x'], 'type': 'Variable'},
               {'arguments': ['y'], 'type': 'Variable'}],
                'type': 'Term'}
...
def test_expressions():
    ...
    # TBox representation
    impl = Implies(And(Term('AncestorOf', dict(ancestor=x, descendant=z)) &
                            Term('AncestorOf', dict(ancestor=z, descendant=y))),
...
@pytest.mark.parametrize("ex1, ex2, eq", [
    ...
    (Implies(Term('p', {'x': 'a1'}), Term('p', {'x': 'a2'})), Implies(Term('p', {'x': 'a1'}), Term('p', {'x': 'a2'})), True),
    (Implies(Term('p', {'x': 'a1'}), Term('p', {'x': 'a2'})), Implies(Term('p', {'x': 'a1'}), Term('p', {'x': 'a3'})), False),
    ])
