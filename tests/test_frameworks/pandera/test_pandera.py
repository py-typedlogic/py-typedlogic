...
from typedlogic import PredicateDefinition, Term, Theory, Variable, axiom
...
def transform(df: DataFrame[Observation]) -> DataFrame[AverageTemperature]:
    ...
    result = (
        temp_df.groupby(Observation.site)[Observation.val]  # specify just the value column
        .mean()
        .reset_index()
...
def test_validate_pandera(example_observation_data: DataFrame[Observation]):
    ...
    solver.add_predicate_definition(
        PredicateDefinition(AverageTemperature.__name__, {"temp_c": "float", "site": "str"})
    )
