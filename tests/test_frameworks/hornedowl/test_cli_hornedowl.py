...
@pytest.mark.parametrize("input_file,input_format", [
    (RO, "owl"),
    (BFO, "owl"),
    (FAMILY_OWLPY, "owlpy"),
    (PATHS_OWLDL, "owlpy"),
...
def test_convert_owl(input_file, input_format, output_format):
    ...
    print(output_path)
    result = runner.invoke(app, ['convert', str(input_file), '--input-format', input_format, '--output-format', output_format, '-o', str(output_path)])
