...
@pytest.mark.parametrize(
    ...
    (PRED_LOGIC_THEORY, [Forall([X], (A1x & Term("eq", X, CONST1)) >> B1x)], [A1], 1, [A1, B1], AllowsComparisonTerms),
    (PRED_LOGIC_THEORY, [Forall([X], (A1x & Term("eq", X, CONST1)) >> B1x)], [A1_2], 1, [A1_2], AllowsComparisonTerms),
    (PRED_LOGIC_THEORY, [Forall([X], (A1x & Term("ne", X, CONST1)) >> B1x)], [A1, A1_2], 1, [A1, A1_2, B1_2], AllowsComparisonTerms),
...
def test_solvers(solver_class, theory, asserted_axioms, asserted_ground_terms, expected_num_models,
                 expected_ground_terms, profile):
    solver = solver_class()
    ...
    if expected_num_models:
        ...
        if expected_num_models > 1 and solver.profile.impl(MultipleModelSemantics):
            assert len(all_models) == expected_num_models, f"Expected {expected_num_models} models, got {len(all_models)}\n{info}"
        model = solver.model()
        assert model
        for f in model.ground_terms:
            print("FACT", f, type(f))
        if solver_class not in (Z3Solver, Prover9Solver) and expected_ground_terms is not None and all(isinstance(f, Term) for f in expected_ground_terms):
            assert set(model.ground_terms) == set(expected_ground_terms), \
                (f"Mismatched axioms: {asserted_axioms}\n{info}")
        for f in expected_ground_terms or []:
            assert solver.prove(f), (f"could not prove {f} using axioms: {asserted_axioms} + \n"
